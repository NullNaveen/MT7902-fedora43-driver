...
/* Moved from mt792x.h due to struct dependency - needs mt7902_vif fully defined */
...
mt792x_sta_to_link(struct mt792x_sta *msta, u8 link_id)
{
	...
	/* Access dev through vif */
	dev = ((struct mt7902_vif *)msta->vif)->phy->dev;
	return rcu_dereference_protected(msta->link[link_id],
		lockdep_is_held(&dev->mt76.mutex));
}
EXPORT_SYMBOL_GPL(mt792x_sta_to_link);
...
{
	struct mt7902_dev *dev = container_of(w, struct mt792x_dev,
					      mt76.tx_worker);
...
{
	struct mt7902_phy *phy = timer_container_of(phy, timer, roc_timer);
...
{
	struct mt7902_vif *mvif = timer_container_of(mvif, timer, csa_timer);
