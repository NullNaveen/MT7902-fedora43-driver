...
struct mt792x_hif_ops {
	int (*init_reset)(struct mt7902_dev *dev);
	int (*reset)(struct mt7902_dev *dev);
	int (*mcu_init)(struct mt7902_dev *dev);
	int (*drv_own)(struct mt7902_dev *dev);
	int (*fw_own)(struct mt7902_dev *dev);
};
...
mt792x_vif_to_link(struct mt792x_vif *mvif, u8 link_id)
{
	...
	/* Simplified - just return bss_conf without RCU protection for now */
	bss_conf = mvif->link_conf[link_id];
	return bss_conf ? bss_conf : &mvif->bss_conf;
}

/* Moved to mt792x_core.c due to struct dependency issues */
struct mt792x_link_sta *
...
static inline bool mt792x_dma_need_reinit(struct mt7902_dev *dev)
{
	/* Simplified stub - mt7902_dev not fully defined here */
	return false;
}
...
void mt792x_mac_reset_counters(struct mt7902_phy *phy);
void mt792x_mac_init_band(struct mt7902_dev *dev, u8 band);
void mt792x_mac_assoc_rssi(struct mt7902_dev *dev, struct sk_buff *skb);
struct mt76_wcid *mt792x_rx_get_wcid(struct mt7902_dev *dev, u16 idx,
				     bool unicast);
void mt792x_mac_update_mib_stats(struct mt7902_phy *phy);
void mt792x_mac_set_timeing(struct mt7902_phy *phy);
...
void mt792x_set_coverage_class(struct ieee80211_hw *hw, int radio_idx,
			       s16 coverage_class);
void mt792x_dma_cleanup(struct mt7902_dev *dev);
int mt792x_dma_enable(struct mt7902_dev *dev);
int mt792x_wpdma_reset(struct mt7902_dev *dev, bool force);
int mt792x_wpdma_reinit_cond(struct mt7902_dev *dev);
int mt792x_dma_disable(struct mt7902_dev *dev, bool force);
...
int mt792x_wfsys_reset(struct mt7902_dev *dev);
...
int mt792x_init_wcid(struct mt7902_dev *dev);
int mt792x_mcu_drv_pmctrl(struct mt7902_dev *dev);
int mt792x_mcu_fw_pmctrl(struct mt7902_dev *dev);
void mt792x_mac_link_bss_remove(struct mt7902_dev *dev,
				struct mt792x_bss_conf *mconf,
				struct mt792x_link_sta *mlink);
void mt792x_config_mac_addr_list(struct mt7902_dev *dev);

static inline char *mt792x_ram_name(struct mt7902_dev *dev)
{
	/* Simplified stub - mt7902_dev not fully defined here */
	return MT7921_FIRMWARE_WM;
}

static inline char *mt792x_patch_name(struct mt7902_dev *dev)
{
	/* Simplified stub - mt7902_dev not fully defined here */
	return MT7921_ROM_PATCH;
}

int mt792x_load_firmware(struct mt7902_dev *dev);

/* usb */
#define MT_USB_TYPE_VENDOR	(USB_TYPE_VENDOR | 0x1f)
#define MT_USB_TYPE_UHW_VENDOR	(USB_TYPE_VENDOR | 0x1e)
int mt792xu_dma_init(struct mt7902_dev *dev, bool resume);
int mt792xu_mcu_power_on(struct mt7902_dev *dev);
int mt792xu_wfsys_reset(struct mt7902_dev *dev);
int mt792xu_init_reset(struct mt7902_dev *dev);
...
static inline void
mt792x_skb_add_usb_sdio_hdr(struct mt7902_dev *dev, struct sk_buff *skb,
			    int type)
{
	u32 hdr, len;

	/* Simplified stub - assume USB for now */
	len = skb->len;
...
int __mt792xe_mcu_drv_pmctrl(struct mt7902_dev *dev);
int mt792xe_mcu_drv_pmctrl(struct mt7902_dev *dev);
int mt792xe_mcu_fw_pmctrl(struct mt7902_dev *dev);

#ifdef CONFIG_ACPI
int mt792x_init_acpi_sar(struct mt7902_dev *dev);
int mt792x_init_acpi_sar_power(struct mt7902_phy *phy, bool set_default);
u8 mt792x_acpi_get_flags(struct mt7902_phy *phy);
u32 mt792x_acpi_get_mtcl_conf(struct mt7902_phy *phy, char *alpha2);
#else
static inline int mt792x_init_acpi_sar(struct mt7902_dev *dev)
{
	return 0;
}

static inline int mt792x_init_acpi_sar_power(struct mt7902_phy *phy,
					     bool set_default)
{
	return 0;
}

static inline u8 mt792x_acpi_get_flags(struct mt7902_phy *phy)
{
	return 0;
}

static inline u32 mt792x_acpi_get_mtcl_conf(struct mt7902_phy *phy, char *alpha2)
{
